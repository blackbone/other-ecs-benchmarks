...
public sealed class LeoEcsLiteContext : IBenchmarkContext<int>
{
    ...
    public void RemoveComponent<T1>(in int[] entities, in int poolId) where T1 : struct, IComponent, IEcsComponent, Friflo.Engine.ECS.IComponent, FFS.Libraries.StaticEcs.IComponent
    {
        var pools = _pools![poolId];
        var p1 = (Leopotam.EcsLite.EcsPool<T1>)pools[0];
        for (var i = 0; i < entities.Length; i++)
    ...
    public bool GetSingle<T1>(in int e, in int poolId, ref T1 c1) where T1 : struct, IComponent, IEcsComponent, Friflo.Engine.ECS.IComponent, FFS.Libraries.StaticEcs.IComponent
    {
        var pools = _pools![poolId];
        var p1 = (Leopotam.EcsLite.EcsPool<T1>)pools[0];
        c1 = p1.Get(e);
